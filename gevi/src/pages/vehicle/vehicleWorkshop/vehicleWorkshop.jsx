...
const WorkshopVehicle = () => {
  ...
  // Toma la mejor fecha de inicio posible desde el objeto del backend
  const getStartMs = (v) => {
    ...
    // 4) reportedAt / reportTimestamp (otros nombres comunes)
    if (v.reportedAt) {
  ...
  // ===== Fetch vehículos en taller =====
  const fetchVehicles = async () => {
    try {
      setFetchError('');
      const token = localStorage.getItem('token');
      const { data } = await axios.get('/api/vehicle-report/workshop', {
        headers: { Authorization: `Bearer ${token}` },
      });
      setVehicles(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error('Error al obtener vehículos del taller:', err);
  ...
  }, [vehicles]); // se reinicia si cambia la lista

  // ===== Sugerencias a partir de la lista cargada =====
  ...
  const filteredVehicles = useMemo(() => {
    ...
    // Orden por más tiempo en taller (más antiguos primero)
          ...
          filteredVehicles.map((v) => {
            ...
            return (
              <div key={v.id} className="vehicleCard" aria-label={`Vehículo ${tag}`}>
                <h3>{tag}</h3>
                <p>
                  <strong>Falla:</strong> {failText}
                </p>
                <p>
                  <strong>Tiempo en taller:</strong>{' '}
